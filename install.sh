...
#
# 使用方法：
#   sudo ./install.sh              # 安装或更新 tcping
#   sudo ./install.sh -u           # 卸载 tcping
#   sudo ./install.sh -f           # 强制安装（跳过确认）
#   sudo ./install.sh -v           # 详细输出
#
...
install_tcping() {
    ...
    # 构造下载 URL - 匹配实际的文件命名格式
    local download_url="$GITHUB_REPO/releases/download/$LATEST_VERSION/tcping-linux-$arch.zip"
    
    print_verbose "下载 URL: $download_url"
    
    # 下载文件
    if ! curl -L -o "tcping.zip" "$download_url"; then
        print_error "下载失败"
        cleanup
        exit 1
    fi
